		...
		for ($percentToSell=$sellMin; $percentToSell<=$sellMax;$percentToSell+=$sellInc) {
			for ($percentToBuy=$buyMin; $percentToBuy<=$buyMax;$percentToBuy+=$buyInc) {
				for ($volatility=$volatilityMin; $volatility<=$volatilityMax; $volatility+=$volatilityInc) {
					for ($emaLong=$emaLongMin; $emaLong<=$emaLongMax; $emaLong++) {
						for ($emaShort=$emaShortMin; $emaShort<$emaLong && $emaShort<=$emaShortMax; $emaShort++) {
							for ($smaLong=$smaLongMin; $smaLong<=$smaLongMax; $smaLong++) {
								for ($smaShort=$smaShortMin; $smaShort<$smaLong && $smaShort<=$smaShortMax; $smaShort++) {
									if ($exportToFile) {
									...
									do {
										...
										if ($exportToFile) {
											file_put_contents('data/test.tsv',
												date('c', $block[0]).'	'.$block[1].'	'.$block[2].'	'.$block[3].'	'.$block[4].'	'.$block[5].'	'.$block[6].'	'.$block[7].'	'.$block[8].'	'.$block[9][$smaShort].'	'.$block[9][$smaLong].'	'.$bits[1].'	'.$bits[2].'	'.$bits[3].'	'.$block[10]."\n",
												FILE_APPEND
											);
										}
									} while ($go_again);
									$current=[
										...
										'smaEmaMix'=>$smaEmaMix,
										'smaShort'=>intval($smaShort),
										'smaLong'=>intval($smaLong),
										'emaShort'=>intval($emaShort),
										'emaLong'=>intval($emaLong),
										'sales'=>$sales,
										'purchases'=>$purchases,
										'holding'=>$bits[1],
									];
